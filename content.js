new function() {
	...
	function showErrorNotification(popupUrl) {
		...
		if(!icon && (options.showIcon || options.showPopup)) {
			...
			icon.src = runtime.getURL('img/error_38.png');
	...
	function codeToInject() {
		...
		window.addEventListener('unhandledrejection', function(e) {
			try{
				...
				if (localStorage['debug']) consoleErrorFunc.apply(console, ["JSD_Error_Notifier - unhandledrejection Failed", ex]);
			}
		});
		...
		window.console.error = function() {
			try{
				...
				// If argsArray contains a Objects with circular references, you may get "cyclic object value" error that will
				// kill the code that calls console.error()
				...
				if (localStorage['debug']) consoleErrorFunc.apply(console, ["JSD_Error_Notifier - Failed", ex]);
			}
		};
		...
		window.addEventListener('error', function(e) {
			try{
				...
				if (localStorage['debug']) consoleErrorFunc.apply(console, ["JSD_Error_Notifier - Dispatch to ErrorToExtension failed", ex]);
			}
		});
		...
		window.addEventListener('error', function(e) {
			try{
				...
				if (localStorage['debug']) consoleErrorFunc.apply(console, ["JSD_Error_Notifier - Dispatch to ErrorToExtension 404 failed", ex]);
			}
		}, true);
	}
